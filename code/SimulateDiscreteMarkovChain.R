...
library(expm)
lineColors = c("indianred","cornflowerblue","forestgreen")
...
# Stationary distribution = left eigenvector of P with eigenvalue of one.
eigObj <- eigen(t(P))
...
x[1] <- 3 # Initial value
for (t in 2:nIter){
  x[t] <- sample(1:nStates, 1, prob = P[x[t-1],])
}
plot(x[1:10])
...
plot(0, type = "n", xlim = c(0,nIter), ylim = c(0,1), 
     ylab = "Marginal probability", xlab = "Iteration number", lwd = 2)
legendText <- c()
for (j in 1:nStates){
  lines(1:nIter, approxStat[[j]], col = lineColors[j], lwd = 2)
  legendText <- c(legendText, paste('Cumulative mean - state',j), 
                  paste('True stationary - state',j))
  abline(piStat[j], 0, col = lineColors[j], lty = 2)
}
legend("topright", inset = 0.05, box.lty = 1, legend = legendText, 
       col = rep(lineColors[1:nStates], each=2), 
